...
export function stopAlert(data: StopAlertData): {
  subject: string
  html: string
} {
  const subject = `🔴 URGENT — Stop Hit: ${data.ticker} at ${fmtPrice(data.currentPrice)}`
  const url = inboxUrl(data.appUrl)

  const html = layout(
    `
    <div style="font-size:28px;font-weight:700;color:#dc2626;">Sell Signal — Stop Hit</div>
    ...
    <div style="margin-top:24px;padding:16px;border-radius:8px;background:#fef2f2;border:1px solid #fecaca;">
      <div style="font-size:16px;font-weight:700;color:#dc2626;">Exit this position in Wealthsimple now</div>
    </div>
    ...
    <table style="width:100%;border-collapse:collapse;margin-top:24px;">
      ...
      <tr><td style="padding:10px 0;color:#737373;">Estimated loss</td><td style="padding:10px 0;text-align:right;font-family:monospace;font-weight:600;">${fmtMoney(data.estimatedLoss)} (${fmtPercent(data.estimatedLossPct)})</td></tr>
    </table>

    <div style="margin-top:28px;">
      <a href="${url}" style="display:inline-block;background:#dc2626;color:#ffffff;text-decoration:none;padding:12px 16px;border-radius:8px;font-weight:600;">
        Open Inbox
      </a>
    </div>
    `
