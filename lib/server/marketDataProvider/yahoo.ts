...
import { mapQuoteToPrice, mapSummaryToFundamentals } from './mapper'
import type {
  FundamentalsData,
  MarketDataProvider,
  MarketIndexData,
...
async function fetchQuoteSafe(ticker: string): Promise<Quote | null> {
  try {
...
async function fetchSummarySafe(ticker: string): Promise<QuoteSummaryResult | null> {
  try {
...
async function fetchMarketIndex(): Promise<MarketIndexData | null> {
  try {
    ...
    const spyPrice = typeof spyQuote?.regularMarketPrice === 'number'
      ? spyQuote.regularMarketPrice
      : null
    const spyVolume = typeof spyQuote?.regularMarketVolume === 'number'
      ? spyQuote.regularMarketVolume
      : null
    const qqqPrice = typeof qqqQuote?.regularMarketPrice === 'number'
      ? qqqQuote.regularMarketPrice
      : null
    const nasdaqPrice = typeof nasdaqQuote?.regularMarketPrice === 'number'
